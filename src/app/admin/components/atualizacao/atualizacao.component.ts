...
export class AtualizacaoComponent implements OnInit {
  ...
  ngOnInit() {
    this.lancamentoId = this.activatedRoute.snapshot.paramMap.get('lancamentoId');
    ...
    this.lancamentoService.buscarPorId(this.lancamentoId)
         .subscribe(
           dados => {
             ...
             this.form.get('id').setValue(dados.data.id);
  ...
  atualizar(){
    ...
    this.lancamentoService.atualizar(this.obterLancamento(dados))
         .subscribe(
           data => {
             let msg: string = 'Lançamento atualizado com sucesso';
  ...
  obterLancamento(dados: any){
    ...
    return new Lancamento(data.format('YYYY-MM-DD HH:mm:ss'),
                          dados.tipo,
                          this.localizacao,
                          this.funcionarioId,
                          this.lancamentoId);
